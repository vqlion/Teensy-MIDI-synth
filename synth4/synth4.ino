...
void recordLoop()
{
  ...
  while (recording && record_index < RECORD_RANGE)
  {
    ...
    if (playingNote)
    {
      ...
      record[record_index] = currentNotePlaying;
    }
    else
    {
      dsp.setParamValue("gate4", 0);
...
void playLoop()
{
  Serial.println(record[loop_index]);
  if (record[loop_index] != -1)
  {
    float frequency = getFrequencyFromMidi(record[loop_index]);
